...
<html>
  ...
  <body>
	   ...
	   <form class="navbar-form navbar-right"  action="" id="searchForm" >
		   ...
		   <input type="text" class="form-control"  id="username-id" name="standardName" placeholder="请输入标准名称">
		   ...
		   <c:if test="${user.role.authority==ROLE_AUTH_ORDINARY || user.role.authority==ROLE_AUTH_SUPER_ADMIN}">
		   	<a  href="javascript:addStandard()" >添加</a>
		   </c:if>
		    </div>
	  </form>
...
function addStandard(){
   layer.open({
	  ...
	  shade: 0.8,
	  area: ['380px', '90%'],
	  content: '${base}/ord/toAddStandard', //iframe的url
	  end:getData
	}); 
} 
...
function getData(){
	$("#searchForm").ajaxSubmit({
		url:"${base}/user/queryStandard_Self",
