...
<html>
  ...
  <body>
	   ...
	   <form class="navbar-form navbar-right"  action="" id="searchForm" >
		   ...
		   <input type="text" class="form-control"  id="username-id" name="rewardName" placeholder="请输入论文名称">
		   ...
		   <c:if test="${user.role.authority==ROLE_AUTH_ORDINARY || user.role.authority==ROLE_AUTH_SUPER_ADMIN}">
		   	<a  href="javascript:addReward()" >申请科技奖励</a>
		   </c:if>
		    </div>
	  </form>
...
function addReward(){
   layer.open({
	  ...
	  shade: 0.8,
	  area: ['380px', '90%'],
	  content: '${base}/ord/toAddReward', //iframe的url
	  end:getData
	}); 
} 
...
function getData(){
	$("#searchForm").ajaxSubmit({
		url:"${base}/user/queryReward_Self",
